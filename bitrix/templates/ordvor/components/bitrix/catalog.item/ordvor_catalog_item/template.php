...
if (isset($arResult['ITEM']))
{
	...
	if($ids && $arResult['ITEM']['OFFERS']){
		 $offerNum = array_usearch($ids, $arResult['ITEM']['OFFERS']);
	}

	
	if($offerNum){
		 $arResult['ITEM']['OFFERS_SELECTED'] = $offerNum;
		 $arResult['ITEM']['DETAIL_PAGE_URL'] .= '?OFFER_ID='.$arResult['ITEM']['OFFERS'][$offerNum]['ID'];
	} 
	...
	<div class="product-item-container<?=(isset($arResult['SCALABLE']) && $arResult['SCALABLE'] === 'Y' ? ' product-item-scalable-card' : '')?>"
		...
		if(isset($item["OFFERS"]) && $item["OFFERS"]) {
			 
			  foreach ($item["OFFERS"] as $i => $offer) {				  
				$codes[$i]['CODE'] = $offer["PROPERTIES"]["KOD"]["VALUE"];
				$codes[$i]['ID'] = $offer["ID"];
			  }
			 unset($offer);
		} 
