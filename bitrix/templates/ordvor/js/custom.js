jQuery(document).ready(function () {
	...
	*/
	...
	let searchMobileButton = document.querySelector('.search-mobile'),
		titleSearch = document.querySelector('#title-search'),
		backgroundSearch = document.querySelector('.search-bg'),
		hideTitleSearch = document.querySelector('.search-hide'),
		classTitleSearchShow = 'title-search-show';
	searchMobileButton.onclick = function () {
		if (titleSearch.classList.contains(classTitleSearchShow)) {
			titleSearch.classList.remove(classTitleSearchShow);
		}
		titleSearch.classList.add(classTitleSearchShow);
	}
	backgroundSearch.onclick = function () {
		titleSearch.classList.remove(classTitleSearchShow);
	}
	hideTitleSearch.onclick = function () {
		titleSearch.classList.remove(classTitleSearchShow);
	}

	function getOpenMenu(selector) {
		let openMenuList = document.querySelectorAll('.' + selector);
		openMenuList.forEach(menuElement => {
			menuElement.classList.remove(selector);
		});
	}

	let buttonMobileMenu = document.querySelector('.mobile-block__button-show'),
		backgroundMobileMenu = document.querySelector('.mobile-block__background'),
		shellMobileMenu = document.querySelector('.mobile-block'),
		hidingButtonMobileMenu = document.querySelector('.mobile-block__hiding-button'),
		classShowMobileMenu = 'mobile-block_show';
	buttonMobileMenu.onclick = function () {
		if (shellMobileMenu.classList.contains(classShowMobileMenu)) {
			shellMobileMenu.classList.remove(classShowMobileMenu);
		}
		shellMobileMenu.classList.add(classShowMobileMenu);
		document.getElementsByTagName("body")[0].classList.add('body__hidden');
	}
	hidingButtonMobileMenu.onclick = function () {
		getOpenMenu('menu-mobile_open');
		shellMobileMenu.classList.remove(classShowMobileMenu);
		document.getElementsByTagName("body")[0].classList.remove('body__hidden');
	}
	backgroundMobileMenu.onclick = function () {
		shellMobileMenu.classList.remove(classShowMobileMenu);
		document.getElementsByTagName("body")[0].classList.remove('body__hidden');
		getOpenMenu('menu-mobile_open');
	}
