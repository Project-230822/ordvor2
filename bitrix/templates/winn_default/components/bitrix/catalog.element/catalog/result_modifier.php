...
if (!is_array($arResult['CATALOGS'][$arResult['IBLOCK_ID']]) && $arResult['MODULES']['redsign.winn'])
{
	$params = array(
...
$component->arParams['LINK_BUY1CLICK'] = SITE_DIR.'buy1click/?RS_ORDER_IDS=#ELEMENT_ID#';
if (
...
if (!$arResult['MODULES']['redsign.winn'] || Option::get('redsign.winn', 'use_sale_order_bonus', 'N', SITE_ID) != 'Y')
{
...
if (!isset($arParams['TAB_PROPERTIES']) || !is_array($arParams['TAB_PROPERTIES']))
{
	$arParams['TAB_PROPERTIES'] = [];
}
...
if (!isset($arParams['BLOCK_LINES_PROPERTIES']) || !is_array($arParams['BLOCK_LINES_PROPERTIES']))
{
	$arParams['BLOCK_LINES_PROPERTIES'] = [];
}
...
if ($arParams['SHOW_CASHBACK'] == 'Y')
{
	if ($haveOffers)
	{
		foreach ($arResult['OFFERS'] as $iOfferKey => $arOffer)
		{
			...
			{
				$arBonus['TYPE'] = Option::get('redsign.winn', 'sale_order_bonus_type', 'F', SITE_ID);
				$arBonus['VALUE'] = Option::get('redsign.winn', 'sale_order_bonus', 'F', SITE_ID);
			}
...
$arSiblingsItems = array();
while ($arElement = $rsElements->GetNext())
{
    $arSiblingsItems[] = $arElement;
}
unset($arElement);
...
if (count($arSiblingsItems) === 3)
{
	$arResult['PREVIOUS_ELEMENT'] = $arSiblingsItems[0];
	$arResult['NEXT_ELEMENT'] = $arSiblingsItems[2];
}
elseif(count($arSiblingsItems) === 2)
{
	if ($arSiblingsItems[0]['ID'] == $arResult['ID'])
	{
		$arResult['NEXT_ELEMENT'] = $arSiblingsItems[1];
	}
	else
	{
		$arResult['PREVIOUS_ELEMENT'] = $arSiblingsItems[0];
