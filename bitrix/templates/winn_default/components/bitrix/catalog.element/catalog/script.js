(function(window){
	...
	window.JCCatalogElement.prototype = {
		...
		init: function()
		{
			...
			this.node.imageZoom = this.getEntity(this.obProduct, 'images-zoom');
...
*/
				...
				if (this.node.imageZoom && !this.isTouchDevice)
				{
					BX.bind(this.node.imageZoom, 'click', BX.proxy(this.zoomImage, this));
				}
...
*/
			...
			{
				this.config.productFeedbackUrl = this.params.CONFIG.LINK_BTN_FEEDBACK;
			}
/*
...
*/
		...
		zoomImage: function()
		{
			...
			var active = this.getEntity(this.product.slider.CONT, 'image', '.active >');
			BX.fireEvent(active, 'click');
						...
						BX.adjust(
							this.obQuantityLimit.all,
							{
								attrs: {
									...
									? 'product-limit is-instock'
									: 'product-limit is-limited',
								},
								style: {display: ''}
							}
						);
...
*/
			...
			for (i = 0; i < images.length; i++)
			{
				...
				img	= BX.create('div', {
					...
					props: {
						className: 'product--detail__slider-image'/* + (i == 0 ? ' active' : '')*/
					},
					children: [img]
				});
/*
...
*/
		...
		checkQuantityControls: function()
		{
			...
			if (reachedTopLimit)
			{
				BX.addClass(this.obQuantityUp, 'product-amount-field-btn-disabled');
			}
			else if (BX.hasClass(this.obQuantityUp, 'product-amount-field-btn-disabled'))
			{
				BX.removeClass(this.obQuantityUp, 'product-amount-field-btn-disabled');
			}

			if (reachedBottomLimit)
			{
				BX.addClass(this.obQuantityDown, 'product-amount-field-btn-disabled');
			}
			else if (BX.hasClass(this.obQuantityDown, 'product-amount-field-btn-disabled'))
			{
				BX.removeClass(this.obQuantityDown, 'product-amount-field-btn-disabled');
			}
			...
			if (this.obPrice.total)
			{
				if (price && this.obQuantity && this.obQuantity.value != this.stepQuantity)
				{
					BX.adjust(this.obPrice.total, {
						...
						+ (
							!!BX.Currency && this.productType > 0
		...
		updatePrice: function(obPriceRow, price)
		{
			...
			if (obPriceCurrent)
			{
				if (price)
				{
					BX.adjust(obPriceCurrent, {html: !!BX.Currency && this.productType > 0 ? BX.Currency.currencyFormat(price.RATIO_PRICE, price.CURRENCY, true) : price.PRINT_RATIO_PRICE});
/*
				...
				if (price && price.RATIO_PRICE !== price.RATIO_BASE_PRICE)
				{
					if (this.config.showOldPrice)
					{
						obPriceFull && BX.adjust(obPriceFull, {
							style: {display: ''},
							html: !!BX.Currency && this.productType > 0 ? BX.Currency.currencyFormat(price.RATIO_BASE_PRICE, price.CURRENCY, true) : price.PRINT_RATIO_BASE_PRICE
						});
/*
...
*/
						...
						if (obPriceDiscount)
						{
							...
							economyInfo = economyInfo.replace('#ECONOMY#', !!BX.Currency && this.productType > 0 ? BX.Currency.currencyFormat(price.RATIO_DISCOUNT, price.CURRENCY, true) : price.PRINT_RATIO_DISCOUNT);
...
*/
				...
				if (obPriceCashBack)
				{
					if (price.RATIO_BONUS > 0)
					{
						...
						this.getEntity(obPriceCashBack, 'cashback-value').innerHTML = !!BX.Currency && this.productType > 0
			...
			if (result.STATUS === 'OK')
			{
				...
				toastr && toastr.info(
					BX.message('COMPARE_MESSAGE_OK')
						.replace('#NAME#', '<b>' + this.product.name + '</b>')
						.replace('#URL#', (this.compareData.comparePath ? this.compareData.comparePath : BX.message('COMPARE_URL')))
				);
/*
...
*/
			...
			else
			{
				...
				toastr && toastr.error(BX.message('COMPARE_UNKNOWN_ERROR'));
/*
				...
				if (arResult.STATUS === 'OK')
				{
					...
					toastr && toastr.info(
						BX.message('TITLE_SUCCESSFUL')
							.replace('#NAME#', '<b>' + this.product.name + '</b>')
							.replace('#URL#', (this.basketData.basketUrl ? this.basketData.basketUrl : BX.message('BASKET_URL')))
					);
/*
					switch (this.productType)
					{
...
*/
				...
				{
					toastr && toastr.error(BX.message('BASKET_UNKNOWN_ERROR'));
/*
					popupContent = '<div style="width: 100%; margin: 0; text-align: center;"><p>'
...
*/
		...
		setCartStatus: function()
		{
			if (this.obBasketActions && window.Basket != undefined)
			{
				...
				if (BX.util.in_array(productID, inbasket))
				{
					...
					if (this.obBuyBtn)
					{
						BX.addClass(this.obBuyBtn, 'btn-dark');
						BX.removeClass(this.obBuyBtn, 'btn-primary');
						this.obBuyBtn.innerHTML = this.mess.MESS_BTN_INCART;
					}
					if (this.obAddToBasketBtn)
					{
						BX.addClass(this.obAddToBasketBtn, 'btn-dark');
						BX.removeClass(this.obAddToBasketBtn, 'btn-primary');
				...
				else
				{
					...
					if (this.obBuyBtn)
					{
						BX.addClass(this.obBuyBtn, 'btn-primary');
						BX.removeClass(this.obBuyBtn, 'btn-dark');
						this.obBuyBtn.innerHTML = this.mess.MESS_BTN_BUY;
					}
					if (this.obAddToBasketBtn)
					{
						BX.addClass(this.obAddToBasketBtn, 'btn-primary');
						BX.removeClass(this.obAddToBasketBtn, 'btn-dark');
		...
		setTimer: function(timer)
		{
			...
			RS.Init(this.obSaleBanner, ['timers'], {
