...
$arParams['SLIDER_SLIDE_COUNT'] = 4;

CJSCore::Init(['popup']);

Loc::loadMessages(__FILE__);

Loader::includeModule('redsign.winn');
...
{
	$measureRatio = 1;
	$actualItem['ITEM_MEASURE']['TITLE'] = Loc::getMessage('RS_WINN_BCI_CATALOG_ITEM_MEASURE');
	$item['ITEM_MEASURE_RATIO_SELECTED'] = 0;
	$item['ITEM_PRICE_SELECTED'] = 0;

	$item['ITEM_MEASURE_RATIOS'][$item['ITEM_MEASURE_RATIO_SELECTED']]['RATIO'] = 1;
	$price = $item['ITEM_PRICES'][$item['ITEM_PRICE_SELECTED']] = $item['RS_PRICES'];
...
*/
	...
	$labelPositionClass = 'product-label-bottom product-label-right';

	$buttonSizeClass = isset($arResult['BIG_BUTTONS']) && $arResult['BIG_BUTTONS'] === 'Y' ? 'btn-md' : '';
	$itemHasDetailUrl = isset($item['DETAIL_PAGE_URL']) && $item['DETAIL_PAGE_URL'] != '';

	$containerClass = 'product';
	switch (strtolower($arResult['TYPE']))
	{
		case 'line':
			$containerClass .= ' product--line';
			break;

		case 'table':
			$containerClass .= ' product--table border-top border-bottom';
			break;
	...
	<article class="<?=$containerClass?>" id="<?=$areaId?>" data-entity="item">
		<?
