...
global $USER;
// if($USER->GetID()=="1814"){
//Банеры для 2го уровня
$arSectionPhoto = [];
$bannerUrl = [];
$rsSections = CIBlockSection::GetList(
    array("UF_SECTION_PHOTO"=>"DESC"),
    array("IBLOCK_ID"=>18, "!UF_SECTION_PHOTO"=>false,"!UF_PATH_OF_BANNER"=>false), false, ["IBLOCK_ID", "ID", "UF_SECTION_PHOTO","UF_PATH_OF_BANNER"]);
while($arSection = $rsSections->Fetch())
{
   $arSectionPhoto[$arSection["ID"]] = CFile::GetPath($arSection["UF_SECTION_PHOTO"]);  
   $bannerUrl[$arSection["ID"]] = $arSection["UF_PATH_OF_BANNER"]; 
}
...
foreach ($arResult["SECTIONS"] as $key => $section)
{
	...
	if ($section["RELATIVE_DEPTH_LEVEL"] == 1)
	{
		...
		$arSections[$section["ID"]] = array(
		        ...
		        "PHOTO"=>$arSectionPhoto[$section["ID"]],
		        "PATH" =>$bannerUrl[$section["ID"]],
				"COUNT" => ''
		);
