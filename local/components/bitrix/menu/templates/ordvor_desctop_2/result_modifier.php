	...
	if ($obCache->InitCache(36000, serialize($arFilter), "/iblock/menu")) {
		...
		if (CModule::IncludeModule("iblock")) {
			...
			if ($arIBlock = $dbIBlock->GetNext()) {
				$dbSections = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arIBlock["ID"]), false, array("*"));
				while ($arSections = $dbSections->GetNext()) {

					$pictureSrc = CFile::GetFileArray($arSections["PICTURE"]);
					$detailPictureSrc = CFile::GetFileArray($arSections["DETAIL_PICTURE"]);
					...
					if ($detailPictureSrc)
						$arResizeDetailPicture = CFile::ResizeImageGet(
							$arSections["DETAIL_PICTURE"],
							array("width" => 240, 'height' => 700),
							BX_RESIZE_IMAGE_PROPORTIONAL,
							true
						);
					...
					$arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["DETAIL_PICTURE"] = $detailPictureSrc ? $arResizeDetailPicture["src"] : false;
...
foreach ($arResult as $key => $arItem) {
	...
	$arItem["PARAMS"]["detail_picture_src"] = $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["DETAIL_PICTURE"];
