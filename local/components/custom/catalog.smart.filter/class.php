...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
    ...
    */
    ...
    function makeFilter($FILTER_NAME)
    {
        ...
        // Добавляем схожие значения
        $gFilter = array_merge($gFilter, $this->makeSynonymGlobalFilter($FILTER_NAME));

        return array_merge($gFilter, $arFilter);
    }

    public function makeSynonymGlobalFilter($FILTER_NAME)
    {
        $resNameProps = CIBlockSection::GetList([], ['IBLOCK_ID' => SYNONYMS_IBLOCK_ID]);
        while ($arNameProps = $resNameProps->GetNext()) {
            $arFilterSyn = array(
                'IBLOCK_ID' => $this->IBLOCK_ID,
                'NAME' => $arNameProps['NAME'],
            );
            ...
            if ($element = $rsProperty->GetNext()) {
                $filterPropName = "=PROPERTY_" . $element['ID'];
                if (!isset($GLOBALS[$FILTER_NAME][$filterPropName])) {
                    continue;
                }
                if ($element['PROPERTY_TYPE'] == 'L') {
                    ...
                    while ($enum_fields = $property_enums->GetNext()) {
                        $arValuesProperty[$enum_fields["ID"]] = $enum_fields["VALUE"];
                    }

                    $searchWords = [];
                    foreach ($GLOBALS[$FILTER_NAME][$filterPropName] as $filterPropValueID) {
                        $searchWords[] = $arValuesProperty[$filterPropValueID];
                    }

                    $arFilterCombined = array("IBLOCK_ID" => SYNONYMS_IBLOCK_ID, 'PROPERTY_WORDS' => $searchWords);
                    $resCombined = CIBlockElement::GetList(array(), $arFilterCombined, false, false, []);
                    while ($arFieldsCombined = $resCombined->GetNextElement()) {
                        $arProps = $arFieldsCombined->GetProperties();
                        foreach ($arProps['WORDS']['VALUE'] as $sinonym) {
                            $findKey = array_search($sinonym, $arValuesProperty);
                            if ($findKey !== false) {
                                $GLOBALS[$FILTER_NAME][$filterPropName][] = $findKey;
                            }
                        }
                    }
                } else {
                    $arFilterCombined = array("IBLOCK_ID" => SYNONYMS_IBLOCK_ID, 'PROPERTY_WORDS' => $GLOBALS[$FILTER_NAME][$filterPropName]);
                    $resCombined = CIBlockElement::GetList(array(), $arFilterCombined, false, false, []);
                    while ($arFieldsCombined = $resCombined->GetNextElement()) {
                        $arProps = $arFieldsCombined->GetProperties();
                        foreach ($arProps['WORDS']['VALUE'] as $sinonym) {
                            $GLOBALS[$FILTER_NAME][$filterPropName][] = $sinonym;
                        }
                    }
                }
            }
