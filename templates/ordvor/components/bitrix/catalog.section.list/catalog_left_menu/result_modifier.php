...
if(CModule::IncludeModule("iblock"))
{
  ...
  if($GLOBALS[$arParams['FILTER_NAME']]){
    $items = GetIBlockSectionList($arParams["IBLOCK_ID"], $arParams["SECTION_ID"], Array("LEFT_MARGIN"=>"ASC"), 0, $GLOBALS[$arParams['FILTER_NAME']]);
  }else{
...
if (!$arResult["SECTIONS"])
{
  ...
  if($GLOBALS[$arParams['FILTER_NAME']]){
    $items = GetIBlockSectionList($arParams["IBLOCK_ID"],  end($arNavChain), Array("LEFT_MARGIN"=>"ASC"), 0, $GLOBALS[$arParams['FILTER_NAME']]);
  }else{
