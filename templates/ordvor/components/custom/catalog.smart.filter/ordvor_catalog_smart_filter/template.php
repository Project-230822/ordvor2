				...
				<?foreach($arResult["ITEMS"] as $key=>$arItem)//prices
				{
					...
					if(isset($arItem["PRICE"])):
						...
						<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-parameters-box bx-active">
							...
							<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)"><span><?=$arItem["NAME"]?> 
							<?php /* ?>
								...
								<div class="row bx-filter-parameters-box-container">
									<div class="col-xs-6 bx-filter-parameters-box-container-block bx-left">
										...
										<div class="bx-filter-input-container">
											<input
												...
												placeholder = "<?=str_replace("&nbsp;", "", $prices[0])?>"
											/>
										</div>
									</div>
									<div class="col-xs-6 bx-filter-parameters-box-container-block bx-right">
										...
										<div class="bx-filter-input-container">
											<input
												...
												size="5"
												onkeyup="smartFilter.keyup(this)"
												placeholder = "<?=str_replace("&nbsp;", "", $prices[4])?>"
											/>
										</div>
									</div>
									...
									<div class="col-xs-12 bx-ui-slider-track-container">
										<div class="bx-ui-slider-track" id="drag_track_<?=$key?>">
											<?for($i = 0; $i <= $step_num; $i++):?>
											<div class="bx-ui-slider-part p<?=$i+1?>"><?php /* ?><span><?=$prices[$i]?></span><?php */ ?></div>
											<?endfor;?>
										...
										<div class="col-xs-12">
											...
											<?foreach ($arItem["VALUES"] as $val => $ar):?>
												...
												if ($ar["DISABLED"])
													$class.= " disabled";
												?>
												<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label <?=$class?> styled-checkbox" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'bx-active');">
													...
													<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label <? echo $ar["DISABLED"] ? 'disabled': '' ?> styled-checkbox" for="<? echo $ar["CONTROL_ID"] ?>">
														<span class="bx-filter-input-checkbox">
															<input
																...
																<? echo $ar["DISABLED"] ? 'disabled': '' ?>
															/>
															<span class="bx-filter-param-text caption" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
															if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
																?>&nbsp;<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span> шт.<?
															endif;?></span>
														</span>
													</label>
												</div>
...
<?ob_start();?>
<?if($arResult["ACTIVE_PROPS2"]){?>

<div class="selected-filter-params" data-entity="selected-filter-params">
	
	<div class="params-list">
	<?foreach($arResult["ACTIVE_PROPS2"] as $row){?>
		<?foreach($row['ITEMS'] as $item){?>
		<div class="params">
		
			<span class="params-value" data-id="<?=$item["CONTROL_ID"]?>" data-elem="item">
				<?=$item['VALUE']?><span class="close-btn"></span>
			</span>
			...
			*/?>
			...
			<?/*if(isset($row['ITEMS']["MIN"]) || isset($row['ITEMS']["MIN"])){?>
				<span class="params-value">
				<?if(isset($row['ITEMS']["MIN"])){?>
					От <?=$row['ITEMS']["MIN"]?>
				<?}?>
				
				<?if(isset($row['ITEMS']["MAX"])){?>
					До <?=$row['ITEMS']["MAX"]?>
				<?}?>
				</span>
			...
			<?}else{?>
				<span class="params-value">
				<?=implode(',',$row['ITEMS'])?><span class="close-btn"></span>
				</span>
			<?}*/?>
...
<?}?>
<?		
	$GLOBALS['FILTER_PARAMS_ACTIVE_TEXT'] = ob_get_contents();
	ob_end_clean();
