...
(function() {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function(parameters)
        {
            ...
            /**/ // Выбираем места, где должны быть купоны
            this.totalCouponNode = BX('totalCoupon');
            this.totalCouponMobileNode = BX('totalCouponMobile');
...
*/
        ...
        editActiveDeliveryBlock: function(activeNodeMode)
        {
            ...
            else
            {
                ...
                /**/ // Добавляем 2 дня к срокам доставки
                /*
				var deliveryElementsCount = deliveryContent.querySelectorAll('.bx-soa-pp-list li .bx-soa-pp-list-description').length;

				var deliveryList = deliveryContent.querySelectorAll('.bx-soa-pp-list li .bx-soa-pp-list-description')[deliveryElementsCount - 1];

				var deliveryListSplit = deliveryList.innerHTML.split('<');

				var last = deliveryListSplit;

				last = last[0].trim().split(' ');
				...
				deliveryListSplit.forEach(function(item)
				{
				...
				deliveryContent.querySelectorAll('.bx-soa-pp-list li .bx-soa-pp-list-description')[deliveryElementsCount - 1].innerHTML = finalHTML;
				*/
                /**/ // Закончили добавлять
        ...
        isValidProperty: function(data, fieldName)
        {
            ...
            /**/ // Добавляем проверку на ФИО в 3 слова
            if (data.inputs[0].id == "soa-property-1")
            {
                var fioString = data.inputs[0].value;
                fioString = fioString.replace(/\s+/g,' ').trim();
                if (fioString.split(" ").length != "3")
                {
                    propErrors[0] = "Укажите фамилию, имя и отчество";
                }
            }
        ...
        editTotalBlock: function()
        {
            ...
            /**/ // Добавляем купоны к тоталу
            $('#totalCoupon').html('');
            this.editCoupons(this.totalCouponNode);
        ...
        editMobileTotalBlock: function()
        {
            ...
            /**/ // Добавляем купоны к мобильному тоталу
            $('#totalCouponMobile').html('');
            this.editCoupons(this.totalCouponMobileNode);
        ...
        totalBlockScrollCheck: function()
        {
            ...
            if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
            {
                ...
                /**/
                this.totalCouponNode.style.width = width + 'px';
                BX.addClass(this.totalCouponNode, 'total-coupon-fixed');
            ...
            else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
            {
                ...
                /**/
                BX.removeClass(this.totalCouponNode, 'total-coupon-fixed');


                this.totalGhostBlockNode.style.paddingTop = 0;
                this.totalInfoBlockNode.style.width = '';

                this.totalCouponNode.style.width = '';
            }
        },
