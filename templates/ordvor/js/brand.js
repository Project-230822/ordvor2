...
function submit_show(left, top)
	...
	value = $.getUrlVar('id');
	if (value != undefined) arr['id'] = value; else arr['id'] = 0;
	...
	$('.catalog_filter').find('.catalog_filter_checklist').each(function(e)
		...
		if (length != 0)
			...
			if (id_feature != 'cat') param = 'param' + id_feature; else param = 'cat';
			arr[param] = '';
...
$(function()
	...
	$("#catalog_sort_type").chosen({no_results_text: "Результаты не соответствуют", width: "156px"});
	$("#catalog_sort_prior").chosen({no_results_text: "Результаты не соответствуют", width: "50px"});
	...
	$('#catalog_filter_clear').on('click', function(e)
		{
		$('.catalog_discount input:checkbox').removeAttr('checked');
