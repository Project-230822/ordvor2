...
if ($arResult['LABEL'] || $arParams['SHOW_DISCOUNT_PERCENT'] === 'Y')
{
	...
	<span class="product-label-text <?=$labelPositionClass?>" id="<?=$itemIds['STICKER_ID']?>">
		...
		{
			?>
			<span class="product-label-text-item" id="<?=$itemIds['DISCOUNT_PERCENT_ID']?>"
		...
		if (!empty($arResult['LABEL_ARRAY_VALUE']))
		{
			foreach ($arResult['LABEL_ARRAY_VALUE'] as $code => $value)
			{
				...
				?>
				<span class="product-label-text-item<?=(!isset($arResult['LABEL_PROP_MOBILE'][$code]) ? ' hidden-xs' : '')?>"<?if (strlen($sLabelStyle) > 0){ echo $sLabelStyle; }?> title=" <?=$value?>">
					<?=$value?>
				</span>
				<?
			}
