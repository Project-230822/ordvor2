(function (window){
	...
	window.JCCatalogItem = function (arParams)
	{
		...
		if (typeof arParams === 'object')
		{
			...
			{
				this.productRequestUrl = arParams.LINK_BTN_FEEDBACK;
			}
...
*/
			...
			{
				this.obProductRequestBtn = BX(this.visual.ACTION_FEEDBACK_LINK);
			}
					...
					for (i in this.offers[index].MORE_PHOTO)
					{
						if (this.offers[index].MORE_PHOTO.hasOwnProperty(i))
						{
							var slide = BX.create('span', {
								props: {className: 'product-image-slide item' + (i == 0 ? ' active' : '')},
								children: [
									BX.create('IMG', {
										props: {className: 'product-image'},
							...
							if (i > this.sliderSlideCount - 1 && this.offers[index].MORE_PHOTO_COUNT > this.sliderSlideCount)
							{
								...
								slide.appendChild(
									BX.create('span', {
										props: {className: 'product-image-slider-more'},
										children: [
											BX.create('span', {
												props: {className: 'product-image-slider-more-wrapper'},
												children: [
													BX.create('span', {
														props: {className: 'product-image-slider-more-icon'},
														html: '<svg class="icon-svg"><use xlink:href="#svg-camera"></use></svg>',
													}),
													BX.create('span', {
														props: {className: 'product-image-slider-more-text'},
														text: photoInfo,
													})
												],
											})
						...
						for (i in this.offers[index].MORE_PHOTO)
						{
							if (this.offers[index].MORE_PHOTO.hasOwnProperty(i))
							{
								this.obPictSliderIndicator.appendChild(
									BX.create('span', {
										attrs: {'data-go-to': i},
										props: {className: 'product-image-slider-control' + (i == 0 ? ' active' : '')}
									})
								);
...
*/
		...
		setPrice: function()
		{
			...
			if (this.obPrice)
			{
				if (price)
				{
					BX.adjust(this.obPrice, {html: !!BX.Currency && this.productType > 0  ? BX.Currency.currencyFormat(price.RATIO_PRICE, price.CURRENCY, true) : price.PRINT_RATIO_PRICE});
				}
				else
				{
				...
				if (this.obPriceTotal)
				{
					if (price && this.obQuantity && this.obQuantity.value != this.stepQuantity)
					{
						BX.adjust(this.obPriceTotal, {
							...
							+ (!!BX.Currency && this.productType > 0 ? BX.Currency.currencyFormat(price.PRICE * this.obQuantity.value, price.CURRENCY, true) : price.CURRENCY.replace('#', BX.util.number_format(price.PRICE * this.obQuantity.value, 0, '.', ' '))),
							style: {display: ''}
						});
					}
					else
