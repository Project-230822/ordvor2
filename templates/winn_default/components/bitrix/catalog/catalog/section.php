...
/*
	...
	{
		if (RS_WINN_CATALOG_SECTION_TYPE == 'type3') 
		{
	...
	if ($isFilter)
	{
			...
			<div class="nav-wrap mt-6" <?=(strlen($arResult['SECTION'][$arParams['LIST_BACKGROUND_COLOR']]) > 0 ? 'style="color:'.$arResult['SECTION'][$arParams['LIST_BACKGROUND_COLOR']].'"' : '')?>>
				<div class="nav">
					...
					foreach ($arResult['SECTION'][$arParams['FEATURE_FILTER_USER_FIELDS']] as $arFeatureFilter)
					{
						...
						{
							?>
							<a class="btn btn-primary mr-2" href="<?=$arResult['SECTION']['SECTION_PAGE_URL'];?>"><?=$arFeatureFilter['UF_NAME']?></a>
							<?php
						}
						else
						{
							?>
							<a class="btn btn-outline-secondary-primary mr-2" href="<?=$arFeatureFilter['UF_LINK'];?>"><?=$arFeatureFilter['UF_NAME']?></a>
							<?php
						}
					}
					?>
		...
		<div class="bg-light py-4 px-2 d-block mb-1 d-md-none">
			<a class="btn btn-outline-secondary w-100" data-fancybox="catalog-filter" data-touch="false" href="#<?=$arParams['AJAX_ID']?>-filter"><?=Loc::getMessage('RS_WINN_BC_CATALOG_FILTER_SHOW')?></a>
		</div>
		<?php
	}
...
*/
	...
	{
		?>
		<div class="mb-1 pb-4 px-lg-4 mt-lg-n6">
			<?php
			...
			$APPLICATION->IncludeComponent(
				...
				array(
					...
					"ALFA_SORT_BY_SHOW" => "N", // $arParams['SORTER_SORT_BY_SHOW'],
					"ALFA_SORT_BY_NAME" => $arParams['SORTER_SORT_BY_NAME'],
					"ALFA_SORT_BY_DEFAULT" => $arParams['SORTER_SORT_BY_DEFAULT'],
					"ALFA_OUTPUT_OF_SHOW" => "N", // $arParams['SORTER_OUTPUT_OF_SHOW'],
			...
			$alfaCOutput = $arParams['PAGE_ELEMENT_COUNT'];
